...
description 'Spark standalone cluster (V2 build)'
...
attribute 'force_spark_reinstall',
          :description  => 'Download and re-install Spark regardless of if the version of the tarball specified above is already installed',
          :default  => 'false',
          :format => {
              :category => '1.General',
              :help => 'Typically on a re-deployment, Spark will only be re-installed on an upgrade, this option forces a re-deployment',
              :order => 2,
              :form => { 'field' => 'checkbox' }
          }
